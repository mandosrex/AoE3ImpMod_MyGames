...
//change log:
//1.01: June 8th, 2014. Fixed broken XML file, removed image paths that direct to a Wotta instalation, removed Wotta triggers, and fixed forests that wouldn't place right. Made sure works in both TAD and Wotta the same way.
//1.00: July 14th, 2012. Release.  Many bugs caused by silly human errors and human ignorance of players who haven't installed Wotta mod.  This is the first map I ever modified.
...
void main(void)
{
   ...
   if (rmAllocateSubCivs(3) == true)
   {
      ...
      rmEchoInfo("subCiv1 is Carib"+subCiv1);
      if (subCiv1 >= 0)
		...
		else
		{
			...
			rmEchoInfo("subCiv2 is Inca"+subCiv2);
			if (subCiv2 >= 0)
				rmSetSubCiv(2, "Incas");
		}
...
//   int smallMapPlayerConstraint=rmCreateClassDistanceConstraint("stay away from players a lot", classPlayer, 70.0);
   ...
   int mediumAvoidImpassableLand=rmCreateTerrainDistanceConstraint("medium avoid impassable land", "Land", false, 18.0);
  ...
  // Text
   ...
   rmAddObjectDefConstraint(socketID, avoidCliff);
	...
	if (tradeRouteNum == 1)
	{
      rmSetObjectDefTradeRouteID(socketID, tradeRouteID);
	   ...
	   rmAddRandomTradeRouteWaypoints(tradeRouteID, 0.4, 1.0, 8, 10);
		...
		vector socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, 0.16);
	...
	else if (tradeRouteNum == 2)
	{
      ...
      rmSetObjectDefTradeRouteID(socketID, tradeRoute2ID);
		// add the meeting sockets along the trade route.
		vector socket2Loc = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.16);
		rmPlaceObjectDefAtPoint(socketID, 0, socket2Loc);

	   socket2Loc = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.5);
		rmPlaceObjectDefAtPoint(socketID, 0, socket2Loc);

	   socket2Loc = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.81);
		rmPlaceObjectDefAtPoint(socketID, 0, socket2Loc);
      ...
      rmSetObjectDefTradeRouteID(socketID, tradeRoute3ID);
		// add the meeting sockets along the trade route.
		vector socket3Loc = rmGetTradeRouteWayPoint(tradeRoute3ID, 0.16);
		rmPlaceObjectDefAtPoint(socketID, 0, socket3Loc);

	   socket3Loc = rmGetTradeRouteWayPoint(tradeRoute3ID, 0.5);
		rmPlaceObjectDefAtPoint(socketID, 0, socket3Loc);

	   socket3Loc = rmGetTradeRouteWayPoint(tradeRoute3ID, 0.81);
		rmPlaceObjectDefAtPoint(socketID, 0, socket3Loc);
	...
	else
	{
      ...
      rmSetObjectDefTradeRouteID(socketID, tradeRoute4ID);
		// add the meeting sockets along the trade route.
		vector socket4Loc = rmGetTradeRouteWayPoint(tradeRoute4ID, 0.16);
		rmPlaceObjectDefAtPoint(socketID, 0, socket4Loc);

	   socket4Loc = rmGetTradeRouteWayPoint(tradeRoute4ID, 0.5);
		rmPlaceObjectDefAtPoint(socketID, 0, socket4Loc);

	   socket4Loc = rmGetTradeRouteWayPoint(tradeRoute4ID, 0.82);
		rmPlaceObjectDefAtPoint(socketID, 0, socket4Loc);
   ...
   if (cNumberTeams == 2)
	...
	if (rmGetNomadStart())
		...
		rmAddObjectDefConstraint(playerMarketID, mediumAvoidImpassableLand);
	...
	rmAddObjectDefConstraint(TCID, mediumAvoidImpassableLand);
   ...
   for(i=1; <cNumberPlayers)
    ...
    rmAddObjectDefItem(playerTreeID, "TreeAmazon", rmRandInt(10,15), 0.0);
   ...
   // Tupis in the south. Caribs or someone else in the north.
...
//      rmAddGroupingConstraint(tupiVillageID, avoidTradeRoute);
   ...
   {  
      int caribVillageID = -1;
      int caribVillageType = rmRandInt(1,5);
      caribVillageID = rmCreateGrouping("carib village", "native carib village "+caribVillageType);
      rmSetGroupingMinDistance(caribVillageID, 0.0);
      rmSetGroupingMaxDistance(caribVillageID, rmXFractionToMeters(0.50));
      rmAddGroupingConstraint(caribVillageID, longAvoidImpassableLand);
      rmAddGroupingToClass(caribVillageID, rmClassID("natives"));
      rmAddGroupingToClass(caribVillageID, rmClassID("importantItem"));
      rmAddGroupingConstraint(caribVillageID, avoidImportantItem);
      rmAddGroupingConstraint(caribVillageID, avoidTradeRoute);
	  rmAddGroupingConstraint(caribVillageID, avoidTownCenter);
		if (tradeRouteNum == 1)
		{
			rmAddGroupingConstraint(caribVillageID, southIslandConstraint);
	      rmPlaceGroupingAtLoc(caribVillageID, 0, 0.25, 0.5);
		}
		else if (tradeRouteNum == 2)
		{
			rmAddGroupingConstraint(caribVillageID, northIslandConstraint);
	      rmPlaceGroupingAtLoc(caribVillageID, 0, 0.85, 0.85);
		}
		else if (tradeRouteNum == 3)
		{	rmAddGroupingConstraint(caribVillageID, northIslandConstraint);
			rmPlaceGroupingAtLoc(caribVillageID, 0, 0.85, 0.85);
   ...
   {   
    ...
    else if(subCiv2 == rmGetCivID("Incas"))
   {   
      int inca2VillageID = -1;
      int inca2VillageType = rmRandInt(1,5);
      inca2VillageID = rmCreateGrouping("inca2 village", "native inca village "+inca2VillageType);
      rmSetGroupingMinDistance(inca2VillageID, 0.0);
      rmSetGroupingMaxDistance(inca2VillageID, rmXFractionToMeters(0.50));
      rmAddGroupingConstraint(inca2VillageID, longAvoidImpassableLand);
      rmAddGroupingToClass(inca2VillageID, rmClassID("natives"));
      rmAddGroupingToClass(inca2VillageID, rmClassID("importantItem"));
      rmAddGroupingConstraint(inca2VillageID, avoidImportantItem);
      rmAddGroupingConstraint(inca2VillageID, avoidTradeRoute);
	  rmAddGroupingConstraint(inca2VillageID, avoidTownCenter);
		if (tradeRouteNum == 1)
		{	rmAddGroupingConstraint(inca2VillageID, southIslandConstraint);
			rmPlaceGroupingAtLoc(inca2VillageID, 0, 0.5, 0.25);
		}
		else if (tradeRouteNum == 3)
		{	rmAddGroupingConstraint(inca2VillageID, northIslandConstraint);
			rmPlaceGroupingAtLoc(inca2VillageID, 0, 0.85, 0.85);
		}
   ...
   for (i=0; <numTries)
      {   
         ...
         rmSetAreaSize(forest, rmAreaTilesToFraction(1800), rmAreaTilesToFraction(4800));
         ...
         rmSetAreaMinBlobs(forest, 24);
         rmSetAreaMaxBlobs(forest, 60);
