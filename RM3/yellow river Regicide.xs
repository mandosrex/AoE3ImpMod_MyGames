...
void main(void)
{
  ...
  whichVersion = 2;   // =========== keep the dry version ================================
...
// Basic map stuff
	...
	if ((weird == true) && (cNumberNonGaiaPlayers > 4))
	size=1.9*sqrt(cNumberNonGaiaPlayers*playerTiles);
...
// Constraints
	...
	int playerConstraint40=rmCreateClassDistanceConstraint("stay away from players by 40", classPlayer, 40.0);
  ...
  // Resource avoidance
	...
	int avoidNuggetFar=rmCreateTypeDistanceConstraint("nugget vs. nugget far", "AbstractNugget", 50.0);
  	...
  	int forestConstraintGroves=rmCreateClassDistanceConstraint("forest vs. forest for small groves", rmClassID("classForest"), 18.0);
	...
	int medAvoidNatives=rmCreateClassDistanceConstraint("stuff avoids natives med", rmClassID("natives"), 35.0);	
...
// Objects
  ...
  int startFoodID=rmCreateObjectDef("starting huntables");
	...
	rmSetObjectDefMaxDistance(startFoodID, 18.0);
      rmAddObjectDefConstraint(startFoodID, avoidAll);
...
*/
      rmRiverAddWaypoint(yellowRiver1, .4, .39);
      rmRiverAddWaypoint(yellowRiver1, .5, .36);
      rmRiverAddWaypoint(yellowRiver1, .6, .39);
...
*/
      rmRiverAddWaypoint(yellowRiver2, .4, .61);
      rmRiverAddWaypoint(yellowRiver2, .5, .64);
      rmRiverAddWaypoint(yellowRiver2, .6, .61);
	...
	if (cNumberNonGaiaPlayers < 4)
         rmRiverAddWaypoint(yellowRiver1, .35, .5);
	else
	...
	if (cNumberNonGaiaPlayers < 4)
         rmRiverAddWaypoint(yellowRiver2, .35, .5);
	else
  ...
  vector socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, 0.06);
  rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);
  
  socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, 0.5);
  rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);

  socketLoc = rmGetTradeRouteWayPoint(tradeRouteID, .94);
  rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);
  ...
  socketLoc = rmGetTradeRouteWayPoint(tradeRoute2ID, 0.06);
  rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);
  ...
  socketLoc = rmGetTradeRouteWayPoint(tradeRoute2ID, .94);
  rmPlaceObjectDefAtPoint(socketID, 0, socketLoc);
...
// Players
   ...
   int playerChoice = rmRandInt(1,5);

  if ( weird == false )  // for normal 2 teams
  {
  // TEAM ONE PLACEMENT RULES
	...
	{
	  if (playerSide == 1) 
           rmPlacePlayersLine(0.36, 0.8, 0.39, 0.8, 0, 0.05);
	  else
           rmPlacePlayersLine(0.36, 0.2, 0.39, 0.2, 0, 0.05);
        nativeALocation = 2;
      }
      else
	{
	  if (playerSide == 1) 
           rmPlacePlayersLine(0.63, 0.8, 0.66, 0.8, 0, 0.05);
	  else
	     rmPlacePlayersLine(0.63, 0.2, 0.66, 0.2, 0, 0.05);
        nativeALocation = 3;
      }
    }
    ...
    {	
      rmSetPlacementTeam(0);
	if (playerSide == 1)
         rmPlacePlayersLine(0.37, 0.80, 0.63, 0.80, 0.1, 0.2);
	else
         rmPlacePlayersLine(0.37, 0.2, 0.63, 0.2, 0.1, 0.2);
    ...
    {	
      rmSetPlacementTeam(0);
	if (playerSide == 1)
         rmPlacePlayersLine(0.27, 0.80, 0.73, 0.80, 0.05, 0.2);
	else
         rmPlacePlayersLine(0.27, 0.2, 0.73, 0.2, 0.05, 0.2);
      rmSetTeamSpacingModifier(0.25);
    }
    else if (teamZeroCount == 3)
    {	
      rmSetPlacementTeam(0);
	if (playerSide == 1)
         rmPlacePlayersLine(0.31, 0.80, 0.69, 0.80, 0.1, 0.2);
	else
         rmPlacePlayersLine(0.31, 0.2, 0.69, 0.2, 0.1, 0.2);

      rmSetTeamSpacingModifier(0.25);
    }
    
    OneVOnePlacement=rmRandFloat(0, 1);
  ...
  {
	int playerPos = rmRandInt(1,3);
	if (playerPos == 1)
	{
    	   rmPlacePlayer(1, .6, .8);
         rmPlacePlayer(2, .6, .2);
         rmPlacePlayer(3, .195, .5);
	}
	else if (playerPos == 2)
	{
         rmPlacePlayer(1, .195, .5);
    	   rmPlacePlayer(2, .6, .8);
         rmPlacePlayer(3, .6, .2);
	}
	else
	{
         rmPlacePlayer(2, .195, .5);
    	   rmPlacePlayer(3, .6, .8);
         rmPlacePlayer(1, .6, .2);
	}
  }
  ...
  {
		rmSetPlayerPlacementArea(0.22,0.08,0.78,0.92);
		rmSetPlacementSection(0.75, 0.5);
		rmPlacePlayersSquare(0.35, 0.00, 0.00);
  }
  else if(cNumberNonGaiaPlayers == 5)
  {
		playerPos = rmRandInt(1,5);
		if (playerPos == 1)
		   rmPlacePlayer(1, .185, .5);
		else if (playerPos == 2)
		   rmPlacePlayer(2, .185, .5);
		else if (playerPos == 3)
		   rmPlacePlayer(3, .185, .5);
		else if (playerPos == 4)
		   rmPlacePlayer(4, .185, .5);
		else if (playerPos == 5)
		   rmPlacePlayer(5, .185, .5);
		rmSetPlayerPlacementArea(0.24,0.09,0.78,0.91);
		rmSetPlacementSection(0.75, 0.5);
		rmPlacePlayersSquare(0.35, 0.00, 0.00);
  }
  else if(cNumberNonGaiaPlayers == 6)
  {
		rmSetPlayerPlacementArea(0.21,0.08,0.78,0.92);
		rmSetPlacementSection(0.75, 0.5);
		rmPlacePlayersSquare(0.38, 0.00, 0.00);
  }
  else if(cNumberNonGaiaPlayers == 7)
  {
		playerPos = rmRandInt(1,7);
 	      if (playerPos == 1)
		   rmPlacePlayer(1, .185, .5);
		else if (playerPos == 2)
		   rmPlacePlayer(2, .185, .5);
		   ...
		   rmPlacePlayer(6, .185, .5);
		else if (playerPos == 7)
		   rmPlacePlayer(7, .185, .5);
		rmSetPlayerPlacementArea(0.22,0.09,0.78,0.91);
		rmSetPlacementSection(0.75, 0.5);
		rmPlacePlayersSquare(0.37, 0.00, 0.00);
  }
  else if(cNumberNonGaiaPlayers == 8)
  {
		rmSetPlayerPlacementArea(0.2,0.08,0.78,0.92);
		rmSetPlacementSection(0.75, 0.5);
		rmPlacePlayersSquare(0.38, 0.00, 0.00);
  }
  ...
  rmSetAreaSize(dryIslandID, 0.03, 0.03);
  ...
  if (subCiv0 == rmGetCivID(nativeOne)) {  
    ...
    rmAddGroupingConstraint(nativeVillage1ID, medAvoidNatives);
  }	
  ...
  if (subCiv1 == rmGetCivID(nativeTwo)) {  
    ...
    rmAddGroupingConstraint(nativeVillage2ID, medAvoidNatives);
  }  
  ...
  if(whichVersion == 2) {
    ...
    rmAddGroupingConstraint(randomNativeID, playerConstraint);
    rmAddGroupingConstraint(randomNativeID, medAvoidNatives);
    rmAddGroupingToClass(randomNativeID, rmClassID("natives"));
    rmPlaceGroupingInArea(randomNativeID, 0, dryIslandID, 1); 
  }

    if(weird && cNumberNonGaiaPlayers == 8) {
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, 0.44, 0.92);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, 0.44, 0.08);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, 0.63, 0.94);
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, 0.63, 0.06);
      rmPlaceGroupingAtLoc(randomNativeID, 0, 0.89, 0.5);
      rmPlaceGroupingAtLoc(randomNativeID, 0, 0.19, 0.5);
      rmPlaceGroupingInArea(nativeVillage1ID, 0, dryIslandID, 1);
      rmPlaceGroupingInArea(nativeVillage2ID, 0, northIslandID, 1);  
    }
    else if(weird && cNumberNonGaiaPlayers == 7) {
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, .28, .85);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, .28, .15);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, .7, .63);
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, .7, .37);
      rmPlaceGroupingInArea(nativeVillage1ID, 0, dryIslandID, 1); 
      rmPlaceGroupingInArea(randomNativeID, 0, northIslandID, 1); 
      rmPlaceGroupingInArea(nativeVillage2ID, 0, northIslandID, 1); 
    }
    else if(weird && cNumberNonGaiaPlayers == 6) {
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, 0.3, 0.9);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, 0.3, 0.1);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, 0.61, 0.93);
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, 0.61, 0.07);
      rmPlaceGroupingAtLoc(randomNativeID, 0, 0.88, 0.5);
      rmPlaceGroupingAtLoc(randomNativeID, 0, 0.2, 0.5);
    }
    else if(weird && cNumberNonGaiaPlayers == 5) {
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, 0.54, 0.91);
      rmPlaceGroupingAtLoc(nativeVillage1ID, 0, 0.54, 0.09);
      rmPlaceGroupingInArea(nativeVillage2ID, 0, dryIslandID, 1); 
      rmPlaceGroupingInArea(nativeVillage1ID, 0, northIslandID, 1); 
      rmPlaceGroupingInArea(nativeVillage2ID, 0, northIslandID, 1);       
    }
    else if(weird && cNumberNonGaiaPlayers == 4) {
	rmPlaceGroupingAtLoc(nativeVillage1ID, 0, .23, .5);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, .5, .92);
      rmPlaceGroupingAtLoc(nativeVillage2ID, 0, .5, .08);
      rmPlaceGroupingInArea(nativeVillage1ID, 0, northIslandID, 1); 
    }
    else if(weird){
   ...
   if(weird == false && cNumberNonGaiaPlayers > 4)
      rmPlaceGroupingInArea(nativeVillage2ID, 0, dryIslandID, 1); 
  ...
  // Some extra natives on the mainland in wet versions
    ...
    rmAddGroupingConstraint(randomNative1ID, medAvoidNatives);
    ...
    rmAddGroupingConstraint(randomNative2ID, medAvoidNatives);
...
// regicide objects
  ...
  rmSetObjectDefMaxDistance(playerDaimyo, 10.0);
   ...
   for(i=1; < cNumberPlayers)
   {
	...
	rmAddObjectDefConstraint(silver1ID, playerConstraint40);
  ...
  rmAddObjectDefConstraint(silver2ID, playerConstraint40);
  ...
  rmAddObjectDefConstraint(silver3ID, playerConstraint40);
...
// Lumber
  ...
  if (cNumberNonGaiaPlayers > 4)
    ...
    rmAddAreaConstraint(forestID, playerConstraint40); 
  ...
  rmAddObjectDefConstraint(nugget1, playerConstraint40);
